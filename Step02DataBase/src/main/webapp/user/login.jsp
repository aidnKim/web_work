...
<%
	...
	//DB 에서 userName 을 이용해서 select 되는 정보가 있는지 select 해본다
	UserDto dto=new UserDao().getByUserName(userName);
	//만일 select 된 정보가 있다면(최소한 userName 은 존재한다는 것)
	if(dto != null){
		// raw 비밀번호와 DB 에 저장된 암호화된 비밀번호를 비교해서 일치하는지 확인한다
	...
	/*
		...
		HttpSession 객체에 저장하면 응답을 한 이후에 다음번 요청에서 HttpSession 객체에
		저장된 정보를 읽어올 수 있다
		세션 객체에 담긴 정보는 어떤 요청도 하지 않고 일정 시간이 흐르면 자동 삭제된다
		필요하다면(로그아웃이 요청되면) 강제로 세션 객체에 담긴 정보를 삭제할 수도 있다
		세션객체에 담긴 로그인 정보를 삭제하는 것이 "로그아웃"이고
		세션객제에 로그인 정보(userName)을 저장하는 것이 "로그인"이다
	*/
	if(isValid){
		//HttpSession 객체에 "userName" 이라는 키값으로 userName 을 저장한다
		session.setAttribute("userName", userName);
		//세션 유지시간 설정(초단위)
		session.setMaxInactiveInterval(60*10); //설정하지 않으면 기본 30분(서버에 30분 동안 어떤 요청도 안갔을 때)
	}
%>
<!DOCTYPE html>
<html>
	...
	<div class="container">
		<%if(isValid){ %>
			<p>
				...
				<a href=<%=url %> %>"${pageContext.request.contextPath }/">확인</a>
			</p>
		<%}else{ %>
			<p>
				아이디 혹은 비밀번호가 틀려요!
