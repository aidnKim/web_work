...
<script>
    ...
    dropZone.addEventListener("drop", (e) => {
        e.preventDefault();
        dropZone.classList.remove("dragover");
        //선택된 파일 객체가 들어 있는 배열을 기능을 모두 쓸 수 있는 배열로 변경한 다음
    ...
    fileInput.addEventListener("change", () => {
        ...
        selectedFiles = selectedFiles.concat(files); // 위의 selectedFiles = [...selectedFiles, ...files]; 와 동일한 기능
        updatePreview();
    });
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            try {
                ...
                btn.addEventListener("click", ()=>{
                	// selectedFiles 배열에서 i 번째 방으로부터 1 개의 item 삭제
    ...
    document.querySelector("#saveForm").addEventListener("submit", (e)=>{
    	//여기서 폼 입력란에 대한 검증을 하고 폼 제출을 막고 싶으면 e.preventDefault() 를 실행하게 하면된다
    	
    	//만일 선택한 파일이 없다면 폼 전송 막기
    	if(selectedFiles.length<1){
    		alert("업로드할 이미지를 1 개 이상 선택해주세요");
    		e.preventDefault();
    	}
    });
